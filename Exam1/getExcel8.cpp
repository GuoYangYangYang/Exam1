...
void getData8(Csv csv, vector<vector<int>> &Class)
{
	...
	if ((csv.getTable()[0][3].compare("科类") == 0) &&
		...
		(csv.getTable()[0][8].compare("文数") == 0) &&
		(csv.getTable()[0][9].compare("英语") == 0) &&
		(csv.getTable()[0][10].compare("政治") == 0) &&
		(csv.getTable()[0][11].compare("历史") == 0) &&
		(csv.getTable()[0][12].compare("地理") == 0)
		)
	{
		int i = 1; // 行数
		...
		if (!csv.getTable().empty()) // 如果原始数据表格不为空
		{
			...
			{
				classN = initClass8(Class, csv.getTable()[i][4]); // 获取班级号码，若数据表中不存在则进行初始化
				if (classN != -1)
				{
					for (int j = 0; j < 7; j++)
					{
